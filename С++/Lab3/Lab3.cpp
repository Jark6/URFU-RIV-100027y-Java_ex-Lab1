...
void quickSortR(int mas[],int size, int t) {
	...
	int S = size;
	int i = 0, j = S-1; // поставить указатели на исходные места
	int temp, p;
	p = mas[S/2]; // центральный элемент
...
//по возрастанию четных и по убыванию нечетных
...
void EvenOddSort(int mas[], int size) {
	...
	//сортировка четных
	//int nE = sizeof(masE) / sizeof(int);
	//int nO = sizeof(masO) / sizeof(int);

	for (i = 0; i < countE; i++) { // i - номер прохода
		for (j = countE - 1; j > i; j--) { // внутренний цикл прохода
			if (masE[j - 1] > masE[j]) {
				x = masE[j - 1];
				masE[j - 1] = masE[j];
				masE[j] = x;
	...
	for (i = 0; i < countO; i++) { // i - номер прохода
		for (j = countO - 1; j > i; j--) { // внутренний цикл прохода
			if (masO[j - 1] < masO[j]) {
				x = masO[j - 1];
				masO[j - 1] = masO[j];
				masO[j] = x;
...
void IndexSort(int mas[], int size) {
	...
	for (i = 0; i < SortSize; i++) { // i - номер прохода
		for (j = SortSize - 1; j > i; j--) { // внутренний цикл прохода
